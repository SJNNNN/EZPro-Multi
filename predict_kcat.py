...
class EnzymeDataset(Dataset):
    def __init__(self, features, kcat_labels=None):
        self.features = features
        self.kcat_labels = kcat_labels
    ...
    def __getitem__(self, idx):
        if self.kcat_labels is None :
            return self.features[idx]

        return (
            self.features[idx],
            self.kcat_labels[idx],
...
def load_test_dataloader(
    ...
    required_cols = ["wt_seq", "mut_seq", "Smiles","log_kcat"]
    for col in required_cols:
        if col not in df.columns:
            raise ValueError(f"Input CSV must contain column: {col}")

    sequences1 = df["wt_seq"].values
    sequences2 = df["mut_seq"].values
    smiles = df["Smiles"].values.tolist()

    has_labels = "log_kcat" in df.columns

    if has_labels:
        kcat_labels = df["log_kcat"].values
    else:
        kcat_labels = None
    ...
    if has_labels:
        dataset = EnzymeDataset(
            features,
            torch.tensor(kcat_labels, dtype=torch.float32),
        )
    else:
...
def inference(model, dataloader, device, has_labels):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Testing"):
            if has_labels:
                data, kcat_label= batch
                true_kcat_list.append(kcat_label.numpy().reshape(-1))
            else:
                data = batch
    ...
    if pred_kcat.ndim > 1:
        pred_kcat_mean = pred_kcat[:, 0].reshape(-1)
    else:
        pred_kcat_mean = pred_kcat.reshape(-1)

    result = {
        "cls_pred": np.asarray(all_cls_preds),
        "cls_prob": np.asarray(all_cls_probs),
        "pred_log_kcat": pred_kcat_mean,
    }

    if has_labels:
        true_kcat = np.concatenate(true_kcat_list, axis=0).reshape(-1)

        rmse = calculate_rmse(true_kcat, pred_kcat_mean)
        spearman = calculate_spearman(true_kcat, pred_kcat_mean)
        pearson = calculate_pearson(true_kcat, pred_kcat_mean)

        print(f"Test RMSE: {rmse:.4f}")
        print(f"Test Spearman: {spearman:.4f}")
        print(f"Test Pearson: {pearson:.4f}")

        result["true_log_kcat"] = true_kcat
...
def main():
    ...
    df["pred_log_kcat"] = result["pred_log_kcat"]
    ...
    if "true_log_kcat" in result:
        df["true_log_kcat"] = result["true_log_kcat"]
