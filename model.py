...
class KcatClassificationModel(nn.Module):
    def __init__(self, rate=0.3, device="cuda:1",pos_embedding_dim=1):
        ...
        # 位置嵌入层：2 个位置（ezy_feats_1 和 ezy_feats_2）
        # self.position_embedding = nn.Embedding(num_embeddings=2, embedding_dim=pos_embedding_dim).to(device)
        ...
        # 解码器，调整输入维度以包含位置嵌入
        # cplx_feats_1 和 cplx_feats_2 的输入维度为 (1024 + pos_embedding_dim) + 768 = 1808
        ...
        # 输出层，调整输入维度以包含位置编码后的特征
        # feats1 和 feats2 的维度为 (cplx_feats + encoder_output) 每个为 (1808 + 256) = 2064
        # 最终 concatenated feats 为 2064 * 2 = 4128
        self.out = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(256, 3),
            nn.Softmax(dim=1)#4098#3584
        ).to(device)  # 3 classes for classification nn.BatchNorm1d(256), nn.Dropout(p=rate),
        self.ddg_out = nn.Sequential(
            nn.Linear(2050, 256),  # 只使用 ezy_feats_1 和 ezy_feats_2（1024 + 1）
            nn.BatchNorm1d(256),
            nn.Dropout(p=rate),
            nn.ReLU(),
            nn.Linear(256, 1),
            nn.Tanh()  # 限制输出在 [-1,1] 区间
        ).to(device)
        ...
        # CrossModalEncoder 实例化
        # 输入维度：query (1024 + pos_embedding_dim), key 和 value (768)
        hidden_dim = 256
        num_heads = 8
        ff_dim = 512
